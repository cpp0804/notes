...
假如线程1执行了前两个步骤，还没有写回主存。同时线程2也执行了前两个步骤并写会了主存，那么当线程1再写会主存就会出错
```java
public class Test {
...
# 2. 实现原理
被volatile修饰的变量，会有一个lock的字节码指令，它相当于一个内存屏障：
1. 指令重排序时，后面的语句不能越过屏障
2. 在将本CPU的缓存写入主存时，也会使得别的CPU中的缓存无效
